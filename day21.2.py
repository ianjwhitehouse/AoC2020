...
for text in fInput:
	...
	while not complete:
		doneOne = False;
		for key, items in nAllergens.items():
			if key not in finalAllergens.keys() and len([i for i in items if i not in usedItems]) == 1 and not doneOne:
				for i in range(len(items)):
					if items[i] not in usedItems:
						finalAllergens[key] = items[i];
						usedItems.append(items[i]);
						doneOne = True;
						break;
		complete = all(key in finalAllergens.keys() for key in nAllergens.keys());
	finalAllergens = dict(sorted(finalAllergens.items()));
	answer = ",".join(finalAllergens.values());
