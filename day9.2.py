...
class VM:
	...
	def runLines(self, lines, ranLimit=100):
		"""
...
for text in fInput:
	...
	for line in text:
		...
		if not getTwoNumbsThatSum(savedVals, val) and len(savedVals) >= preamble:
			r = getRangeOfNumbsThatSum(allSavedVals, val);
			answer = min(r) + max(r);
			break;
		savedVals.append(val);
		allSavedVals.append(val);
		if len(savedVals) > preamble:
			del savedVals[0];
