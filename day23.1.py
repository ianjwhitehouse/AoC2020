...
for text in fInput:
	...
	for actions in range(100):
		...
		pickUpCups = [];
		for ii in range(3):
			if cups.index(curCup)+1 < len(cups):
				pickUpCups.append(cups.pop(cups.index(curCup)+1));
			else:
				pickUpCups.append(cups.pop(0));
		desCup = curCup -1;
		while desCup in pickUpCups or desCup not in cups:
			desCup -= 1;
			if desCup < min(cups):
				desCup = max(cups);
		cups = cups[:cups.index(desCup)+1] + pickUpCups + cups[cups.index(desCup)+1:];
		if cups.index(curCup)+1 < len(cups):
			curCup = cups[cups.index(curCup)+1];
		else:
			curCup = cups[0]
